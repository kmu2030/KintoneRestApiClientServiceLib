...
<#
# About This Script
This script stops the KintoneRestApiClientService running in the controller.
...
# このスクリプトについて
このスクリプトは、コントローラで動作するKintoneRestApiClientServiceを停止します。
パラメータは、スクリプトの引数または、環境変数で渡します。

UserIdentityのパスワードが気になるようであれば、扱いを修正してください。
...
try {
    ...
    & {
        $service.Stop($client.Session)
        do { Start-Sleep -Milliseconds 200 } while ($service.IsBusy($client.Session))
    } | Out-Null
